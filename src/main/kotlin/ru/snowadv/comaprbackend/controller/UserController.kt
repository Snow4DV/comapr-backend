...
class UserController(
    ...
    fun getActiveSessions(@RequestHeader("Authorization") authorizationHeader: String?): ResponseEntity<Any> {
        val user = SecurityContextHolder.getContext().currentUserIdOrNull()?.let { userService.getUserById(it) } ?: return ResponseEntity.status(401).body(MessageResponse("not_authorized"))
        val userDto = converter.userToDto(user)
        val sessionsDto = sessionService.getAllSessionsByUser(user.id ?: error("not persisted user")).map { converter.mapSessionToDto(it) }
